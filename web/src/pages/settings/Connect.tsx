  ...
  const inner = (
    <>
      ...
      <Section title="App Store Connect API 凭证">
        {ascConfigured && !editing ? (
          <div className="list">
            <ListRow
              leading={<span className="row-icon tone-success"><Icon name="check" size={16} /></span>}
              title="已配置"
              detail="Key ID · Issuer ID · 私钥 · Vendor（敏感值只写不回显）"
              trailing="chevron"
          ...
          <>
            ...
            <div className="field">
              ...
              <textarea id="asc-key" rows={4} value={ascKey} onChange={(e) => setAscKey(e.target.value)} placeholder={ascConfigured ? '已配置，如需更换请粘贴新私钥' : ''} />
            </div>
            <div className="field">
            ...
            {ascConfigured ? (
              <button className="ghost btn-block mt-3" onClick={() => setEditing(false)}>取消</button>
            ) : (
              <p className="muted hint" role="status">未配置（可选，用于拉取可回复评论与账单报告）</p>
            )}
          </>
        )}
      </Section>
